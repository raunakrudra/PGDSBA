...
#View(loandt.train)
#df_fin
#loandt.train <- loandt.train[,-c(13:14)]
```
...
#Calculating AUC and KS for selected model on test data
loandt.test$predict.class=predict(ptree,loandt.test,type="class")
loandt.test$predict.score=predict(ptree,loandt.test)
...
library(pROC)
roc_obj1 = roc(loandt.test$PersonalLoan, loandt.test$predict.score[,2])
...
```{r}
#Performance measures on Train Data

loandt.train$predict.class = NULL
loandt.train$predict.score = NULL
loandt.train$prediction = NULL
loandt.train$predict.prob1 = NULL

loandt.train$prediction = predict(ptree, loandt.train, type="class")
loandt.train$prob1 = predict(ptree, loandt.train, type="prob")[,"1"]
#View(loandt.train)


probs=seq(0,1,length=11)
qs=quantile(loandt.train$prob1, probs)
loandt.train$deciles=cut(loandt.train$prob1, unique(qs),include.lowest = TRUE,right=FALSE)
table(loandt.train$deciles)

library(data.table)
trainDT = data.table(loandt.train)
...
library(ROCR)
library(ineq)
predObj = prediction(loandt.train$prob1, loandt.train$PersonalLoan)
...
auc
print("   ")
gini = ineq(loandt.train$prob1, type="Gini")
print("Gini")
gini
...
Concordance(actuals=loandt.train$PersonalLoan, predictedScores=loandt.train$prob1)
...
loandt.test$predict.class = NULL
loandt.test$predict.score = NULL
loandt.test$prediction = NULL
loandt.test$predict.prob1 = NULL

loandt.test$prediction = predict(ptree, loandt.test, type="class")
loandt.test$prob1 = predict(ptree, loandt.test, type="prob")[,"1"]
View(loandt.test)


probs=seq(0,1,length=11)
qs=quantile(loandt.test$prob1, probs)
loandt.test$deciles=cut(loandt.test$prob1, unique(qs),include.lowest = TRUE,right=FALSE)
table(loandt.test$deciles)

library(data.table)
testDT = data.table(loandt.test)
rankTbl1 = testDT[, list(
  cnt = length(PersonalLoan), 
  cnt_tar1 = sum(PersonalLoan == 1), 
  cnt_tar0 = sum(PersonalLoan == 0)
  ), 
...
rankTbl1$ks = abs(rankTbl$cum_rel_resp - rankTbl1$cum_rel_non_resp);
...
#library(ROCR)
#library(ineq)
predObj = prediction(loandt.test$prob1, loandt.test$PersonalLoan)
perf = performance(predObj, "tpr", "fpr")
plot(perf)
KS = max(perf@y.values[[1]]-perf@x.values[[1]])
print("KS")
...
auc
print("   ")
gini = ineq(loandt.test$prob1, type="Gini")
print("Gini")
gini
...
#install.packages('InformationValue')
library(InformationValue)
Concordance(actuals=loandt.test$PersonalLoan, predictedScores=loandt.test$prob1)
